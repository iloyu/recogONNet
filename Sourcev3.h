...
{Mat pre;
	...
	dst=(float*)malloc(sizeof(float)*h_windowx*h_windowy*my_feature);
 ...
 fclose(fp);
 float svm_bias=0,*svm_weights,t_fAugRate=1;
 svm_weights=(float *)malloc(sizeof(float)*2160);
 memset(svm_weights,0,sizeof(float)*2160);
 vector <CvRect> t_vTarget;
 int svm_count=2160;
 countFeaturesfloat(dst,m_fNormalMat,m_fMagMat,m_ANGImage,m_MagImage,
	 mask,hist_mask, svm_bias,svm_weights, svm_count, width, height,
	  t_fAugRate,t_vTarget);
 //countFeaturesfloat(dst,m_fNormalMat,m_fMagMat,m_ANGImage,m_MagImage,width,height,mask,hist_mask,32,32);
