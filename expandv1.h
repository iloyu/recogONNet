...
__global__ void right(float *in,float *out,float *device_c_ANG,float *device_c_Mag,float *device_p_ANG,float *device_p_Mag,int ImageHeight,int Imagewidth,int *d_mask,int *d_hist_mask){
 ...
 extern "C" void countFeaturesfloat(uchar *in,float *out,int *d_ANG,int *d_Mag,float *c_ANG,float *c_Mag,float *p_ANG,float *p_Mag,int Imagewidth,int ImageHeight,int *mask,int *histo_mask)
{
 ...
 //       for(int j=0;j<h_windowx;j++)
	//	{ */      //smoothcell<<<block_smooth,threads_smooth>>>(device_out+(i*h_windowy+j)*1260,device_smooth_out);
		   ...
		   //cudaMemcpy(device_smooth_in,device_out,size_s_cell,cudaMemcpyDeviceToDevice);
			
	 //countCell<<<blocks,threads>>>( device_out, ImageHeight,Imagewidth);
