...
__global__ void countCell(uchar *in,float *out,int *d_ANG,int *d_Mag,float *c_ANG,float *c_Mag,float *p_ANG,float *p_Mag,int ImageHeight,int Imagewidth)
{
    ...
    //for(int stridex=0,stridey=0;stridex<gridDim.x, )
            ...
            //if(tidx3<128&&tidy3<128)
			 ...
			 __syncthreads();
				atomicAdd(& (histo[d_ANG[tidy1*m_nImage+tidx1]*70+d_Mag[tidy1*m_nImage+tidx1]*10+t_nm_nbin1]),p_Mag[yy1*ImageHeight+xx1]);
			 __syncthreads();
				atomicAdd(& (histo[d_ANG[tidy2*m_nImage+tidx2]*70+d_Mag[tidy2*m_nImage+tidx2]*10+t_nm_nbin2]),p_Mag[yy2*ImageHeight+xx2]);
			 __syncthreads();
				atomicAdd(& (histo[d_ANG[tidy*m_nImage+tidx]*70+d_Mag[tidy*m_nImage+tidx]*10+t_nm_nbin]),p_Mag[yy*ImageHeight+xx]);
            __syncthreads();
